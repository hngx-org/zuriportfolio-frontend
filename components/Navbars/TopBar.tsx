...
function TopBar(props: { activePage: string; showDashBorad: boolean }) {
  ...
  return (
    <nav className="w-full py-6  bg-white-100 border-b border-[#EBEEEF] justify-between items-center px-4  z-[1] relative ">
      ...
      <MenuUI cart={6} toggle={toggle} toggler={handleToggle} />
    </nav>
  );
}
...
function MenuUI({
  ...
  return (
    <nav
      className={`menu lg:hidden bg-white-100 h-[100dvh] absolute top-0 left-0 right-0 ${
      ...
      <ul className="p-4 flex gap-4 flex-col items-center">
        <div className="justify-start items-center gap-4 flex">
          ...
          >
            <select
              id="explore"
              className="text-zinc-900 text-base font-normal  pr-7 leading-normal tracking-tight appearance-none focus:border-0 focus:outline-none focus:ring-0
                   hover:cursor-pointer  bg-brand-green-ttr px-6 py-3 "
            >
              <option className="hover:cursor-pointer hover:bg-orange-800">Explore</option>
              <option className="hover:cursor-pointer hover:bg-orange-800">Marketplace</option>
            </select>
        ...
        <div className="max-w-sm w-[100%] h-auto lg:h-12 p-4 rounded-lg border border-neutral-200 justify-start items-center gap-3 flex lg:flex-row flex-col">
          <div className="grow shrink basis-0 h-6 justify-start items-center gap-2 flex lg:w-[20dvw] w-auto">
            <div className="w-4 h-4 justify-center items-center flex">
              <div className="w-4 h-4 relative">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 16 16">
