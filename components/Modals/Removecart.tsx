...
const RemoveCart: React.FC<RemoveCartProps> = ({ closeModal, onRemoveItem, productId }) => {
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-[#00000080] bg-opacity-30">
      <div
        id="modal"
        className="bg-white-100 p-4 rounded-lg w-[90%] md:w-[50%] lg:w-[24%] h-415 text-center duration-[50ms] animate-slideIn"
      >
        <svg
          onClick={closeModal}
        ...
        <div className="flex flex-row items-between justify-between mt-5">
          <button className="px-4 py-2 border rounded-md flex items-center gap-2" onClick={closeModal}>
          ...
          <button
            className="px-4 py-2 bg-[#DE3730] text-[#FFFFFF] text-[14px] flex items-center gap-2 rounded-md ml-4"
            onClick={() => {
              closeModal();
