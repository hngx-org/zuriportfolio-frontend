...
const OutOfTime: React.FC<OutOfTimeProps> = ({ onClose, onRetake }) => {
  return (
    <div className='w-fit m-auto items-center font-manropeB justify-center bg-transparent rounded-[16px] p-4 gap-3 flex flex-col'>
      ...
      <button onClick={onClose} className="bg-transparent text-brand-green-hover hover:bg-brand-green-hover hover:text-white-100 rounded-[16px] px-8 py-2"> Go to my Dashboard</button>
      <button onClick={onRetake} className="bg-brand-green-primary text-[1.1em] hover:bg-brand-green-hover rounded-[16px] text-white-100 px-8 py-2"> Retake assessment</button>
