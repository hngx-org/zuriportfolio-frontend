...
const ExpiredPromo: React.FC = () => {
  return (
    <div
      className={`md:bg-[#FFDCDC] text-[#FF2E2E] ${manropeMD.className} mx-auto py-[2px] pl-[6px] pr-2 flex items-center gap-2 w-fit rounded-2xl `}
    >
...
const PromotionHistoryRow = (props: PromotionHistory) => {
  ...
  return (
    <>
      <tr className="font-manropeL border-[#EAECF0] border rounded-lg text-center text-[16px] font-normal text-[#667085] [&>*]:px-6  [&>*]:py-4">
        <td className="text-left">{props.productName}</td>
        <td className={`text-[#101828] ${manropeMD.className}`}>{props.type}</td>
        <td>
          {props.status === 'active' && <ActivePromo />}
          {props.status === 'expired' && <ExpiredPromo />}
          {props.status === 'deactivated' && <DeactivatedPromo />}
        </td>
        <td className={`text-[#101828] ${manropeMD.className}`}>{props.discount}</td>
        <td className={`text-[#101828] ${manropeMD.className}`}>{props.quantity}</td>
        <td className={`text-[#101828] ${manropeMD.className}`}>{props.sales}</td>
        <td className={`text-[#101828] ${manropeMD.className}`}>
      ...
      {isDeleteModalOpen && (
        <Modal isOpen={isDeleteModalOpen} size="lg" closeModal={toggleDeleteModal} title="">
          <div className="flex flex-col text-center p-10">
            <h2 className="text-dark text-[24px] font-manropeEB px-20 leading-8 mt-2">
              Are you sure you want to delete this coupon?
            </h2>
            <p className="text-dark font-manropeL text-[16px] mt-2">
              Coupon wil be permanently deleted from your store.
            </p>
            <button className="bg-[#FF2E2E] text-white-100 font-manropeL p-2 rounded-lg mt-5" onClick={handleDelete}>
              Delete Coupon
            </button>
            <button className="border border-[#009254] p-2 text-[#009254] rounded-lg mt-3" onClick={toggleDeleteModal}>
              Cancel
            </button>
          </div>
        </Modal>
  ...
  return (
    ...
    >
      <div className="flex flex-col">
        ...
        <p className="text-[12px] mb-3  text-[#2E3130]">{props.type}</p>
      </div>
      <div className="flex flex-col">
        <td className={`text-[#101828] ${manropeMD.className}`}>
          <button onClick={toggleDeleteModal}>
