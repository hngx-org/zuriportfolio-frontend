...
const FilterDropDown = ({onChooseFilter}: {onChooseFilter: (filter: string) => void}) => {
    ...
    return (
        ...
        <Menu>
          ...
          <Menu.Items className="py-2 mt-[.5rem] text-sm font-medium text-gray-700 border border-slate-100">
            {
                filterByList.map(item => (
                    <Menu.Item key={item.id}>
                    {({ active }) => (
                        <span
                        className={`block px-4 py-2 capitalize ${active ? 'bg-white-200 text-white' : 'text-gray-700'
                            }`}
