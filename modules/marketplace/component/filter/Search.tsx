...
import { category, discount, keyword, price, rating, subCategory } from './data/searchData';
import FilterSection from './filter-section';
import Button from '@ui/Button';
import { CancelIcon } from './icons';
import Badge from './Badge';

const Search = ({ isOpen, toggle }: { isOpen?: boolean; toggle: () => void }) => {
  return (
    <Fragment>
      {isOpen ? (
        <div className="flex items-center justify-center absolute z-50  w-full top-0 min-h-screen bg-black bg-opacity-50 py-10 backdrop-blur-sm transition duration-300 ease-in-out">
          <div className="bg-white-100 max-w-2xl w-full py-5 px-7 rounded-md shadow-sm transition delay-500 ease-in-out">
